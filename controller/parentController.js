...
exports.getBulletinEnfant = async (req, res) => {
    try {
        ...
        // Appréciations des professeurs — même mécanisme que côté élève.
        ...
        `, trimestreDemande ? [enfantId, trimestreDemande] : [enfantId]);
        ...
        // Rang dans la classe — même mécanisme que côté élève.
        ...
        try {
            ...
            if (idsClasse.length) {
                ...
                const lui = classes.find(c => c.id === enfantId);
                const valides = classes.map(c => c.moyenne).filter(m => m !== null);
                rangInfo = {
                    position: lui ? lui.rang : null,
        ...
        } catch (e) { console.error('Calcul rang classe (parent):', e.message); }
...
// ✅ Bulletin PDF de l'enfant — même vérification d'accès que
// getBulletinEnfant, même document que côté élève (bulletinService).
exports.getBulletinEnfantPdf = async (req, res) => {
    try {
        const parentId = req.user?.id;
        const enfantId = req.query.enfant_id;
        if (!parentId || !enfantId) return res.status(401).json({ message: 'Données manquantes' });
        ...
        if (accessCheck.rows.length === 0) return res.status(403).json({ message: 'Accès non autorisé' });
