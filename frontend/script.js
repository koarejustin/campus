...
function initDashboard(role, context, userData) {
    ...
    // 🔒 CLOISONNEMENT : chaque rôle va sur SA page dédiée
    if (role === 'PARENT')     { window.location.replace('parent.html');     return; }
    if (role === 'PROFESSEUR') { window.location.replace('professeur.html'); return; }
    if (role === 'ALUMNI')     { window.location.replace('alumni.html');     return; }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (params.get('auth') === 'success') {
        // Retry jusqu'à 10x / 100ms — corrige le bug "ressort instantanément"
        let tentatives = 0;
        function essayer() {
            tentatives++;
            const sessionData = localStorage.getItem('user_session');
            if (sessionData) {
                initDashboard(params.get('role'), params.get('context'), JSON.parse(sessionData));
            } else if (tentatives < 10) {
                setTimeout(essayer, 100);
            } else {
